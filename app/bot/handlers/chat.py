...
async def _generate_and_send(
    ...
    """Общий пайплайн: бюджет -> LLM (с tools) -> ответ + вложения -> учёт."""
    ...
    # В группах отвечаем reply'ем на сообщение, к которому обращались
    sent = await reply_rendered(
        message, text, as_reply=workspace.type == WorkspaceType.group
    )
